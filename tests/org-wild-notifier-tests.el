...
;;; Tests for auto-add org-id feature

(ert-deftest auto-add-org-id-when-enabled ()
  "Tests that org-id is automatically added to entries without one.
When `org-wild-notifier-auto-add-org-id' is non-nil (default), entries
without an ID should get one assigned during gather-info."
  (let ((temp-file (make-temp-file "org-wild-notifier-test-" nil ".org")))
    (unwind-protect
        (progn
          ;; Create a temp org file with an entry without ID
          (with-temp-file temp-file
            (insert "* TODO Test entry without ID\n")
            (insert "  SCHEDULED: <2018-01-04 Thu 16:00>\n"))
          ;; Now gather info with auto-add enabled
          (cl-letf* ((org-agenda-files (list temp-file))
                     (org-todo-keywords (list "TODO" "DONE"))
                     (time-string "January 4, 2018 15:50")
                     ((symbol-function 'current-time)
                      (lambda () (apply 'encode-time (parse-time-string time-string))))
                     ...
                     (org-wild-notifier-predicate-whitelist nil)
                     (org-wild-notifier-predicate-blacklist '(org-wild-notifier-done-keywords-predicate))
                     (org-wild-notifier-auto-add-org-id t))
            (let ((events (org-wild-notifier--gather-events-sync)))
              ;; Should have exactly one event
              (should (= (length events) 1))
              ;; Event should have an id
              (should (assoc 'id (car events)))
              (should (stringp (cdr (assoc 'id (car events)))))
              ;; The file should now contain the ID
              (with-temp-buffer
...
(ert-deftest no-auto-add-org-id-when-disabled ()
  "Tests that org-id is NOT added when feature is disabled.
When `org-wild-notifier-auto-add-org-id' is nil, entries without an ID
    ...
    (unwind-protect
        (progn
          ...
          (with-temp-file temp-file
            (insert "* TODO Test entry without ID\n")
            (insert "  SCHEDULED: <2018-01-04 Thu 16:00>\n"))
          ;; Now gather info with auto-add disabled
          (cl-letf* ((org-agenda-files (list temp-file))
                     (org-todo-keywords (list "TODO" "DONE"))
                     (time-string "January 4, 2018 15:50")
                     ((symbol-function 'current-time)
                     ...
                     (org-wild-notifier-predicate-whitelist nil)
                     (org-wild-notifier-predicate-blacklist '(org-wild-notifier-done-keywords-predicate))
                     (org-wild-notifier-auto-add-org-id nil))
            (let ((events (org-wild-notifier--gather-events-sync)))
              ;; Should have exactly one event
              (should (= (length events) 1))
              ;; Event should NOT have an id
              (should-not (cdr (assoc 'id (car events))))
              ;; The file should NOT contain an ID
              (with-temp-buffer
                (insert-file-contents temp-file)
...
(ert-deftest auto-add-org-id-preserves-existing-id ()
  "Tests that existing org-ids are preserved and not overwritten."
  (let ((temp-file (make-temp-file "org-wild-notifier-test-" nil ".org"))
        (existing-id "my-existing-unique-id-12345"))
    (unwind-protect
        (progn
          ;; Create a temp org file with an entry WITH an existing ID
          (with-temp-file temp-file
            (insert "* TODO Test entry with existing ID\n")
            (insert "  SCHEDULED: <2018-01-04 Thu 16:00>\n")
            (insert "  :PROPERTIES:\n")
            (insert (format "  :ID: %s\n" existing-id))
            (insert "  :END:\n"))
          ...
          (cl-letf* ((org-agenda-files (list temp-file))
            ...
            (let ((events (org-wild-notifier--gather-events-sync)))
              ;; Should have exactly one event
              (should (= (length events) 1))
              ;; Event should have the existing id
              (should (equal (cdr (assoc 'id (car events))) existing-id)))))
      ;; Cleanup
      (delete-file temp-file))))
